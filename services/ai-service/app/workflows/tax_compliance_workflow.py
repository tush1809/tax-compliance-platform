...
# Define state as TypedDict for better LangGraph compatibility
...
def retrieve_context(state: TaxComplianceState) -> TaxComplianceState:
    query = state["user_data"].get('query', 'tax compliance rules')
    query_embedding = embedder.embed(query)
    results = vector_store.search(query_embedding, top_k=3)
    state["retrieved_context"] = '\n'.join([doc for doc, score, id in results])
    return state

# Define nodes (steps)
def calculate_tax(state: TaxComplianceState) -> TaxComplianceState:
...
def validate_rules(state: TaxComplianceState) -> TaxComplianceState:
...
def run_workflow(user_data):
    initial_state = TaxComplianceState(
        user_data=user_data,
        calculation_result={},
        validation_result={},
        retrieved_context=""
    ...
    # Return dictionary format expected by the API
    return {
